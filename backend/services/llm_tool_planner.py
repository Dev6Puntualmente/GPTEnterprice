...
2. NUNCA escribas SQL ni pseudo-código. NUNCA expliques "puedes usar la función X" — EJECÚTALA tú.
3. Responde ÚNICAMENTE con un JSON válido en una sola línea (sin markdown):
...
def run_llm_tool_planner(
    ...
    if not allowed:
        return run_agent(messages=messages, system_prompt=system_prompt, tools=None, vllm=vllm)
    ...
    parsed = _extract_json_object(plan.get("message") or "")
    ...
    if not parsed:
        return {
            ...
            "model_used": plan.get("model_used") or "planner",
            "tool_calls": None,
            "files": None,
        }
